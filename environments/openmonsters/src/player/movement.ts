...
import EnemyMovement from "../enemies/movement.ts";
...
export default class PlayerMovement extends Behavior {
  ...
  onTickServer(): void {
    ...
    if (move.t === "move") {
      ...
      this.bombPrefab.cloneInto(this.game.world, { transform: { position: this.#pos, z: 20 } });
    }
  }
  ...
  placeBomb(): { success: boolean; error?: string } {
    if (!this.canMoveYet()) return { success: false, error: "can't place yet!" };

    if (!this.bombPrefab) return { success: false, error: "missing bomb prefab" };

    const inventory = this.entity.getBehavior(InventoryBehavior);
    if (inventory.bombs === 0) return { success: false, error: "no bombs available" };
  ...
  teleportTo(position: Vector2): void {
    ...
    this.entity.pos.assign(this.#pos);
  }
  ...
  #tileCheck(tile: IVector2): boolean {
    ...
    return color !== Colors.Wall && color !== Colors.BombWall && color !== Colors.GlassWall;
  }
  ...
  #canPushBlock(tile: IVector2, direction: IVector2): boolean {
    ...
    if (color !== Colors.PushableBlock && color !== Colors.BlockOnGoal) return true;
  ...
  vision() {
    ...
    const colorSymbols: Record<number, string> = {
      ...
      [Colors.BlockOnGoal]: "p", // block on goal (success!)
    };
    ...
    const enemies = this.game.world.entities.lookupByBehavior(EnemyMovement);
    const enemyPositions: Record<string, Entity> = {};
    for (const e of enemies) {
      enemyPositions[keyOf(e.pos.x, e.pos.y)] = e;
    }
    ...
    const collectibles = this.game.world.entities.lookupByBehavior(CollectibleBomb);
