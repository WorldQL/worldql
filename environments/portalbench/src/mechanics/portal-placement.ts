...
export default class PortalPlacement extends Behavior {
  ...
  #leftClick = this.inputs.create(
    "@portal/placeBlue",
    "Place Blue Portal",
    "MouseLeft"
  );
  #rightClick = this.inputs.create(
    "@portal/placeOrange",
    "Place Orange Portal",
    "MouseRight"
  );
  #togglePortal = this.inputs.create(
    "@portal/toggle",
    "Toggle Portal Color",
    "KeyQ"
  );
  ...
  onTick(): void {
    ...
    if (this.#togglePortal.pressed) {
      this.currentPortalColor =
        this.currentPortalColor === "blue" ? "orange" : "blue";
    }
